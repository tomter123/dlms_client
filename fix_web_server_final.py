...
post_start = c.find('static esp_err_t post_handler(httpd_req_t *req) {')
post_end = c.find('static esp_err_t api_data_handler', post_start)

new_post = """static esp_err_t post_handler(httpd_req_t *req) {
...
c = c[:post_start] + new_post + c[post_end:]
